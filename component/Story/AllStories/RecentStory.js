...
const RecentStory = ({ type, fetchData, storyPage }) => {
  ...
  const getRecentStory = useCallback(async () => {
    setloader(true);

    const data = await getAll(`storyApi/recent/${type}?page=${storyPage}&user_id=${user?.user_id}`);
    // console.log(data);
