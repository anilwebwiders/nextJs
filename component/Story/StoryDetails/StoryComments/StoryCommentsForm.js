...
const StoryCommentsForm = ({ story_id, getComments, scroll }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const post = { message: commentTxt, story_id, user_id: auth.user.user_id };
    const res = await postData(`storyApi/comment/main`, post, auth.token);
    ...
    <div className="article-comments-write">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="comment-post">
          ...
          <span>{commentTxt.length}/500 Words
          </span>
        </div>
      </form>
    </div>
