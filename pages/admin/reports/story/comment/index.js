...
const AdminStoryReportsComment = ({}) => {
  ...
  const get = useCallback(async () => {
    if (auth.user.user_id) {
      setLoading(true);
      const storiesComment = await getFetchData(
        `adminApi/reports/story/comment/main?status=${status}&page=${page}`,
        auth.token
      );

      setCount(storiesComment.count);
      setData([...data, ...storiesComment.storyComment]);
      setLoading(false);
    }
  }, [page, status]);
        ...
        <div className="nav-admin text-center">
          <div className="d-flex justify-content-center">
            <h3 className="admin-box-inactive bg-danger">
              ...
              Story Comments Reports: {loading ? <Preloader /> : count}
            </h3>
          </div>
        </div>
                  ...
                  {banLoader === rep.story_id ? (
                    ...
                    <div className="dropdown">
                      <select
                        ...
                        onChange={() =>
                          deleteStoryComment(rep.story_comment_id)
                        }
                      >
                        <option
...
export default AdminStoryReportsComment;
